...
if __name__=='__main__':
    ...
    try:
        ...
        dicke = MeanFieldDickeTEMPO(sp, expbath, finf = finf, bath_tr = bath_tr, bath_0 = bath_0)
    except:
        dicke = MeanFieldDickeTEMPO(sp, expbath)
    ...
    np.savetxt(os.path.join(outpath, "dynamics_ohmic.csv"), np.column_stack((sp.t_list, np.real(a_t), np.imag(a_t), np.abs(a_t)**2, x_t, y_t, z_t)), delimiter=',')
