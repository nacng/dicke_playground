...
def svd_truncate(tensor: np.ndarray, cutoff: float, linds: List[int], rinds: Optional[List[int]] = None, p: Optional[float] = 2.0, maxdim: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    description
    ...
    S_trunc = s[0:trunc_dim] #np.diag(s[0:trunc_dim])
